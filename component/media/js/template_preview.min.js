...
((window, document) => {
  const onSocialMagickSampleClick = e => {
    ...
    document.getElementById("socialMagicSampleDescription").innerText = Joomla.Text._("COM_SOCIALMAGICK_TEMPLATE_LBL_SAMPLE_IMAGE_OPT_" + sampleKey.toUpperCase());
  ...
  const onSocialMagickRegenerateTemplatePreview = async e => {
    ...
    window.localStorage.setItem("socialmagick_preview_textdebug", document.getElementById("socialMagickPreviewTextDebug").checked ? "1" : "0");
    const response = await fetch("index.php?option=com_socialmagick&view=templates&task=regeneratePreview&format=json", {
      method: "POST",
      body: formData
    });
    if (!response.ok) {
...
//# sourceMappingURL=template_preview.min.js.map